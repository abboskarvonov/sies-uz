        ...
        <div class="container mx-auto px-4 lg:px-0 py-12 lg:py-16">
                ...
                <div class="space-y-4">
                    <div class="flex items-center gap-2">
                        <img src="{{ $siteSettings?->logoUrl() ?? asset('img/logo.webp') }}"
                            alt="{{ $siteSettings?->siteName() ?? __('messages.app_name') }}"
                    ...
                    {{-- Ijtimoiy tarmoqlar (SVG iconlar) --}}
                    <div class="flex space-x-3 pt-4">
                        @if ($siteSettings?->telegram_url)
                            <a href="{{ $siteSettings->telegram_url }}" target="_blank" aria-label="Telegram">
                                {{-- Telegram brand SVG --}}
                                <img src="{{ asset('img/icons/telegram.webp') }}" alt="Telegram icon"
                                    class="w-7 h-7 dark:invert" width="28px" height="28px" />
                            </a>
                        @endif

                        @if ($siteSettings?->facebook_url)
                            <a href="{{ $siteSettings->facebook_url }}" target="_blank" aria-label="Facebook">
                                <img src="{{ asset('img/icons/facebook.webp') }}" alt="Facebook icon"
                                    class="w-7 h-7 dark:invert" width="28px" height="28px" />
                            </a>
                        @endif
                    ...
                    <div
                        ...
                        <p class="flex items-center gap-3 text-sm lg:text-base text-gray-700 dark:text-gray-300">
                            <img src="{{ asset('img/icons/placeholder.webp') }}" alt="Address icon"
                                class="w-6 h-6 dark:invert" width="24px" height="24px" />
                            {{ $address }}
                        </p>
                        ...
                        <p class="flex items-center gap-3 text-sm lg:text-base text-gray-700 dark:text-gray-300">
                            ...
                            {{ __('messages.phone') }}:
                            @if ($phonePrimary)
                                <a href="tel:{{ $phonePrimary }}" class="hover:underline">{{ $phonePrimary }}</a>
                                @if ($phoneSecondary)
                                    ,
                                    <a href="tel:{{ $phoneSecondary }}"
                                        class="hover:underline">{{ $phoneSecondary }}</a>
                                @endif
                            @else
                                +998 (66) 231-12-53
                            @endif
                        ...
                        <p class="flex items-center gap-3 text-sm lg:text-base text-gray-700 dark:text-gray-300">
                            ...
                            Email:
                            @if ($emailPrimary)
                                <a href="mailto:{{ $emailPrimary }}" class="hover:underline">{{ $emailPrimary }}</a>
                                @if ($emailSecondary)
                                    ,
                                    <a href="mailto:{{ $emailSecondary }}"
                                        class="hover:underline">{{ $emailSecondary }}</a>
                                @endif
                            @else
                                sies_info@sies.uz
                            @endif
            ...
            <div class="mt-10 pt-6 border-t border-gray-400 dark:border-gray-600 text-gray-600 dark:text-gray-400">
                <p class="text-sm flex items-center gap-2">
                    {{-- Alert SVG --}}
                    <svg class="w-4 h-4 shrink-0 dark:invert" fill="none" stroke="currentColor" stroke-width="2"
